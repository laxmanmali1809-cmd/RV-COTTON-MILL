...
- Admin quick loader to edit an existing PO and add an image
...
6. To add an image for an existing PO, open the admin dashboard, use `Edit existing PO`, paste/upload the image, and save.

## Image Note

The admin dashboard can save pasted/uploaded PO images to the running server so client cards can display them.

For permanent production image storage, the best method is still:

1. Upload image to Google Drive or cloud storage.
2. Paste the public image link in the Google Sheet `IMAGE URL` column.

This avoids losing images if a free hosting server restarts.
