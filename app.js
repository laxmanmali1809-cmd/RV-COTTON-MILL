...
const ORDER_IMAGE_ENDPOINT = "/api/order-images";
...
const els = {
  ...
  quickEditPo: document.getElementById("quickEditPo"),
  quickEditClient: document.getElementById("quickEditClient"),
...
async function importGoogleSheetOrders({ silent = false, force = true, keepClientCode = false, retrying = false } = {}) {
  ...
  try {
    ...
    const response = await fetch(`${ADMIN_ORDERS_ENDPOINT}${force ? "?force=true" : ""}`, {
      cache: "no-store",
    ...
    const importedOrders = (payload.orders || []).map((order) => ({
      ...order,
...
async function saveOrderImageOverride(order, { retrying = false } = {}) {
  const pin = getAdminPin({ promptOnMissing: false });
  const response = await fetch(ORDER_IMAGE_ENDPOINT, {
    method: "POST",
    headers: {
  ...
  if (response.status === 401 && !retrying) {
    const nextPin = getAdminPin({ promptOnMissing: true });
    if (nextPin) {
      return saveOrderImageOverride(order, { retrying: true });
    }
  }

  if (!response.ok) {
    throw new Error("Image override save failed");
  }
}
...
async function handleOrderSubmit(event) {
  ...
  const fileImage = file ? await readImageFile(file) : "";
  const hasNewImage = Boolean(stagedPastedImage || fileImage);
  const image = stagedPastedImage || fileImage || existing?.image || textileImageForOrder(els.clientCode.value, els.fabric.value || "Fabric");
  ...
  if (hasNewImage) {
    try {
      await saveOrderImageOverride(order);
    } catch (error) {
      console.error(error);
      showToast("Image saved locally, but server image publish failed.");
    }
  }
...
function quickEditExistingPo(event) {
  event.preventDefault();
  const po = String(els.quickEditPo.value || "").trim().toLowerCase();
  const code = normalizeCode(els.quickEditClient.value);

  if (!po) {
    showToast("Enter a PO number to load.");
    return;
  }

  const matches = state.orders.filter((order) => {
    const poMatches = String(order.poNo || "").trim().toLowerCase() === po;
    const codeMatches = !code || order.clientCode === code;
    return poMatches && codeMatches;
  });

  if (!matches.length) {
    showToast("PO not found. Import Google Sheet first or check the code.");
    return;
  }

  editOrder(matches[0].id);
  if (matches.length > 1 && !code) {
    showToast("Multiple matches found. Loaded first match; add client code for exact PO.");
  } else {
    showToast(`Loaded PO ${matches[0].poNo}. Add image and save.`);
  }
}
...
els.quickEditForm.addEventListener("submit", quickEditExistingPo);
